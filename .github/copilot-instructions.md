...
This is the **ntfy-client** adapter — an unofficial [ntfy.sh](https://ntfy.sh) client for ioBroker. ntfy is a simple HTTP-based pub-sub notification service that lets you send notifications to your phone or desktop directly from scripts, without registering an account. The adapter enables ioBroker users to:

- Subscribe to ntfy topics and receive push notifications via Server-Sent Events (SSE)
- Publish notifications to topics with rich options (priority, tags, click URL, attachments, email, phone call, custom actions)
- Support self-hosted ntfy servers as well as the public ntfy.sh service
- Authenticate via no-auth, username/password, or access token
- Manage multiple subscribed topics simultaneously
- Dismiss or delete received notifications
- Use Blockly for visual scripting of notifications

---

## Adapter-Specific Context

- **Adapter Name:** ntfy-client
- **Primary Function:** Unofficial ntfy.sh client — subscribes to topics via SSE and publishes notifications with rich options
- **Key Dependencies:** `axios` (HTTP client for publishing/API calls), `eventsource` (SSE subscription to ntfy topics)
- **Authentication Types:** `none` (public topics), `user` (username + password Basic Auth), `token` (Bearer token)
- **Configuration Requirements:** Server URL, authType, credentials (user/pass or token), defaultTopic, and a list of subscribed topics
- **Protected/Encrypted Fields:** `pass` and `token` are encrypted in ioBroker's native config
- **Connection Type:** Cloud (connects to public ntfy.sh or a self-hosted instance)
- **Data Flow:** Topics are subscribed via SSE streams; notifications are published via HTTP POST to the ntfy API

### Key Implementation Patterns

- **SSE Subscriptions:** Each subscribed topic gets its own `EventSource` connection stored in `this.subscriptions` (a `Map`)
- **Config Signature:** A SHA-256 hash of server + auth config is stored locally to detect config changes and trigger reconnections
- **Throttled Logging:** Warn/error logs are throttled (`warnLogState`, `sseErrorLogState`) to avoid log spam during connection issues
- **Stats Interval:** Account statistics are polled periodically via `this.statsInterval`
- **Message Handling:** `onMessage()` processes `sendTo()` commands for publishing notifications from scripts/Blockly

### ntfy API Specifics

- **Base URL:** Configurable (default: `https://ntfy.sh`)
- **Publish endpoint:** `POST {url}/{topic}` with JSON body or headers
- **Subscribe endpoint:** `GET {url}/{topic}/sse` (Server-Sent Events stream)
- **Authentication header:** `Authorization: Basic ...` or `Authorization: Bearer ...`
- **Rich publish options:** `title`, `message`, `priority` (1–5), `tags`, `click`, `attach`, `filename`, `email`, `call`, `actions`
- **Notification management:** DELETE requests to dismiss/delete notifications by ID

### Error Handling Patterns

- Reconnect SSE streams on connection errors with throttled error logging
- Validate auth configuration before attempting connections
- Log only the auth type (never credentials) in connection logs
- Handle HTTP error codes from ntfy API (401 Unauthorized, 403 Forbidden, 429 Rate Limited)
- Gracefully handle self-hosted server connectivity issues
...
<!-- ntfy-client adapter-specific coding standards -->
The ntfy-client adapter uses `axios` for HTTP requests (publishing to ntfy API, fetching account stats) and `eventsource` for SSE topic subscriptions. When contributing:
- Keep SSE subscription management centralized through the `subscriptions` Map
- Always close/delete existing EventSource connections before re-subscribing
- Use throttled logging helpers (`warnLogState`, `sseErrorLogState`) for connection-related warnings
- Never log plaintext credentials; log only the `authType` when debugging auth issues
- Validate the ntfy server URL by stripping trailing slashes before use
