...
This adapter (`ntfy-client`) is an **unofficial client for the [ntfy.sh](https://ntfy.sh) push notification service**. It allows ioBroker to:
- Subscribe to ntfy topics via Server-Sent Events (SSE) and surface received notifications as ioBroker states.
- Publish notifications with rich options (title, message, priority, tags, click URL, attachment, filename, email, phone call, actions, etc.).
- Manage account statistics and dismiss/delete notifications.
- Use Blockly visual programming for sending notifications.

Key characteristics:
- Runtime code lives in `main.js` (CommonJS style — use `require`/`module.exports`, not ESM `import`/`export`).
- Admin UI configuration is JSON-Config based in `admin/jsonConfig.json`.
- Blockly support is implemented in `admin/blockly.js`.
- Translation files are in `admin/i18n/*.json` (11 languages).
- Authentication supports three modes: `none`, `user`/`pass` (HTTP Basic), and `token` (Bearer token).
- Sensitive fields `pass` and `token` are declared in both `encryptedNative` and `protectedNative` in `io-package.json` — never remove these declarations.
- SSE subscriptions are managed via the `eventsource` npm package; all active connections are tracked in `this.subscriptions` (a `Map`).
- HTTP publish calls use `axios`; do not switch to `fetch` unless migration is explicitly requested.
...
> **ntfy-client exception:** This adapter uses `axios` for HTTP publish calls. Do not switch to `fetch` here unless migration is explicitly requested.
...
## ntfy-client Adapter-Specific Patterns

### Module Style
- Keep CommonJS (`require`/`module.exports`) throughout `main.js` — do not migrate to ESM unless explicitly requested.
- Node.js `>=20` is required (see `package.json` `engines` field).

### SSE Subscription Lifecycle
- Active subscriptions are tracked in `this.subscriptions` (`Map<topicId, EventSource>`).
- Before re-subscribing to a topic, always close and remove the existing `EventSource` to avoid leaks.
- In `onUnload()`, close **all** active `EventSource` connections and clear `this.subscriptions`.
- Use `this.sseErrorLogState` and `this.warnLogState` Maps for throttled error/warn logging to prevent log floods.

### Authentication Header Construction
- Never log the value of `pass`, `token`, or the `Authorization` header — only log the authentication *type*.
- Use `this.getConfiguredAuthType()` to get the canonical auth type (`none`, `user`, or `token`).
- Use `this.sanitizeHeadersForLog()` to strip sensitive values before any log output.

### Config Signature
- `this.getServerAccountConfigSignature()` produces a SHA-256 hash of server+account config.
- Compare against the stored signature to detect changes that require re-initialization (close all SSE connections, reset states, re-subscribe).

### Stats and Timers
- `this.statsInterval` drives periodic account stats fetching — always clear it in `onUnload()`.
- `this.versionCheckTimer` is used for deferred version checks — always clear it in `onUnload()`.

### Sensitive Native Fields
- `pass` and `token` are declared in both `encryptedNative` and `protectedNative` in `io-package.json`.
- Never remove these declarations; they ensure credentials are encrypted at rest and masked in the admin UI.

### ioBroker State and Object Conventions
- Keep `io-package.json` and `package.json` versions consistent on every release.
- Preserve and update `common.news` entries in `io-package.json` for every user-visible change.
- All states must follow ioBroker conventions: correct `role`, `read`/`write` flags, and `def` values.

### Pull Request Checklist
- `npm run lint` passes (zero warnings).
- `npm run test:package` and `npm run test:integration` pass.
- README updated if behavior or configuration changed.
- `io-package.json` `common.news` reviewed for release-relevant changes.
- No secrets or credentials in code, logs, tests, or docs.
- Translation files updated if `admin/jsonConfig.json` labels/help texts changed.
