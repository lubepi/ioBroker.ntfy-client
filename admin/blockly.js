...
if (typeof Blockly !== 'undefined') {
    ...
    Blockly.Words['ntfy_manage_tooltip'] = {
        ...
        'zh-cn': '管理 ntfy-client 消息。\n行动：标记为已读并驳回或删除。\n主题：Topic。\n序列ID：唯一消息ID或序列ID。',
    };

    Blockly.Blocks['ntfy_mutator_container'] = {
        init: function () {
            this.appendDummyInput().appendField(Blockly.Words['ntfy_mutator_arguments'][systemLang]);
    ...
    const ntfyBooleanOptions = ['disable_cache', 'disable_firebase', 'unified_push', 'markdown'];

    ntfyMutatorOptions.forEach(function (opt) {
        Blockly.Blocks[`ntfy_mutator_${opt.key}`] = {
            init: function () {
                this.appendDummyInput('ATTR').appendField(Blockly.Words[opt.word][systemLang]);
    ...
    Blockly.Blocks['ntfy'] = {
                ...
                .appendField(
                    new Blockly.FieldDropdown(function () {
                        if (typeof main !== 'undefined' && main.instances) {
                            ...
                            var result = [[Blockly.Words['ntfy_anyInstance'][systemLang], '']];
                            ...
                            return result.length > 1 ? result : [[Blockly.Words['ntfy_anyInstance'][systemLang], '']];
                        }
        ...
        mutationToDom: function () {
            ...
            container.setAttribute(
                'with_statement',
                withStatement === true || withStatement === 'true' || withStatement === 'TRUE',
            );
            return container;
        },
        ...
        decompose: function (workspace) {
            ...
            if (flyout && !flyout._ntfySquashed) {
                ...
                if (flyout.workspace_ && typeof flyout.workspace_.getCanvas === 'function') {
            ...
            const capBubbleAndScroll = () => {
                ...
                if (ownerSvg) {
                    ...
                    if (maxScroll > 0) {
                        ...
                        if (!sbGroup) {
                            ...
                            if (flyout.svgBackground_) {
                                const updateThemeColors = () => {
                                    ...
                                    try {
                                        const cFill = window.getComputedStyle(flyout.svgBackground_).fill;
                                    ...
                                    track.setAttribute('fill', isDarkLocal ? '#ffffff' : '#000000');
                                    track.setAttribute('fill-opacity', isDarkLocal ? '0.2' : '0.12');
                                    thumb.setAttribute('fill', isDarkLocal ? '#ffffff' : '#000000');
                                    thumb.setAttribute('fill-opacity', isDarkLocal ? '0.6' : '0.45');
                                };
                        ...
                        sbGroup.setAttribute('transform', `translate(${flyoutX + flyoutW - 10}, ${flyoutY + 12})`);

                        const trackHeight = maxVisibleHeight - 24;
                        track.setAttribute('height', String(trackHeight));

                        const thumbHeight = Math.max(20, (maxVisibleHeight / contentHeight) * trackHeight);
                ...
                if (!svgGroup._ntfyWheelAttached) {
                    ...
                    svgGroup.addEventListener(
                        ...
                        e => {
                            ...
                            if (bc) {
                                bc.setAttribute('transform', `translate(0, ${-scrollY}) scale(1)`);
                            }

                            if (svgGroup._ntfyThumb) {
                                const thumbY =
                                    (scrollY / max) * (svgGroup._ntfyTrackHeight - svgGroup._ntfyThumbHeight);
            ...
            workspace.addChangeListener(e => {
                if (
                    ...
                    if (flyout) {
                        if (availStr !== lastAvailableBlocksStr && typeof flyout.show === 'function') {
        ...
        updateShape_: function (withStatement) {
            ...
            for (const attrName of this.attributes_) {
                ...
                if (!input) {
                    if (isBoolean) {
                        input = this.appendDummyInput(attrName).setAlign(Blockly.ALIGN_RIGHT);
                        input.appendField(fieldText).appendField(new Blockly.FieldCheckbox('FALSE'), fieldName);
                    } else if (isPriority) {
                        input = this.appendDummyInput(attrName).setAlign(Blockly.ALIGN_RIGHT);
                        ...
                        input = this.appendValueInput(attrName).setAlign(Blockly.ALIGN_RIGHT);
    ...
    Blockly.Blocks['ntfy_manage'] = {
        init: function () {
            ...
            this.appendValueInput('TOPIC').setCheck('String').appendField(Blockly.Words['ntfy_topic'][systemLang]);
            ...
            this.setHelpUrl('https://ntfy.sh/docs/publish/#updating-deleting-notifications');
        },
    ...
    Blockly.JavaScript['ntfy_manage'] = function (block) {
        ...
        var value_id = Blockly.JavaScript.valueToCode(block, 'SEQUENCE_ID', Blockly.JavaScript.ORDER_ATOMIC);
        ...
        var instance = dropdown_instance ? `"${dropdown_instance}"` : '"ntfy-client.0"';
        ...
        let logCode = '';
        if (dropdown_log) {
            const logStr = logParts.length ? logParts.join(' + ", " + ') : '"[no args]"';
    ...
    Blockly.JavaScript['ntfy'] = function (block) {
        ...
        const instance = dropdown_instance ? `"${dropdown_instance}"` : '"ntfy-client.0"';
        ...
        for (const attrName of block.attributes_) {
            ...
            if (ntfyBooleanOptions.includes(attrName)) {
                ...
                vArgument = block.getFieldValue(`FIELD_${attrName}`) === 'TRUE' ? 'true' : 'false';
            } else if (attrName === 'priority') {
                const prioVal = block.getFieldValue(`FIELD_${attrName}`);
                vArgument = `"${prioVal}"`;
            } else {
                // Logic for value inputs (texts, variables)
                vArgument = Blockly.JavaScript.valueToCode(block, attrName, Blockly.JavaScript.ORDER_COMMA);
            ...
            if (vArgument !== undefined && vArgument !== null && vArgument !== 'null') {
                args.push({
        ...
        const argStr = args.length
            ? args
                  .map(a => Blockly.JavaScript.prefixLines(`'${a.attr}': ${a.val},`, Blockly.JavaScript.INDENT))
                  .join('\n')
            : '';
        ...
        if (dropdown_log) {
            ...
            const logParts = logArgs.map(a => `'${a.attr}: ' + ${a.val}`).join(' + ", " + ');
